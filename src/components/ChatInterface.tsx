        ...
        <div className="w-80 bg-gray-50 border-r border-gray-200 flex flex-col">
          ...
          <div className="px-4 pb-4">
            <button
              ...
              className="w-full bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white px-4 py-3 rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-[1.02] flex items-center justify-center gap-2 font-medium"
            >
          ...
          {/* Conversations List */}
          <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-2 border-t border-gray-200 pt-4 min-h-0">
            {filteredConversations.length === 0 ? (
              ...
              filteredConversations.map((conversation) => (
                <div
                  key={conversation.id}
                  className={`group p-3 rounded-xl cursor-pointer transition-all duration-200 ${currentConversationId === conversation.id
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <h4
                      className={`font-semibold text-sm truncate flex-1 ${currentConversationId === conversation.id ? 'text-blue-800' : 'text-gray-900'
                        }`}
                    >
                      {conversation.title}
                    </h4>
                  ...
                  <div
                    className={`flex items-center gap-2 text-xs ${currentConversationId === conversation.id ? 'text-blue-600' : 'text-gray-500'
                      }`}
                  >
                  ...
                  {conversation.messages.length > 1 && (
                    <p
                      className={`text-xs mt-2 truncate ${currentConversationId === conversation.id ? 'text-blue-700' : 'text-gray-600'
                        }`}
                    >
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50 min-h-0">
                {messages.map((message) => (
                  ...
                  >
                    <div
                      className={`flex gap-4 max-w-4xl ${message.type === 'user' ? 'flex-row-reverse' : 'flex-row'
                        }`}
                    >
                      {/* Avatar */}
                      <div className="w-10 h-10 rounded-full flex items-center justify-center shadow-md bg-gradient-to-br from-blue-500 to-indigo-600">
                        {message.type === 'user' ? (
                      ...
                      <div className={`group relative ${message.type === 'user' ? 'text-right' : 'text-left'}`}>
                        {/* Message bubble */}
                        <div
                          className={`inline-block px-6 py-4 ${message.type === 'assistant' ? 'pb-10' : ''
                            } rounded-2xl shadow-sm ${message.type === 'user'
                        ...
                        {/* Footer : horodatage à gauche, boutons à droite (si assistant) */}
                        <div className="mt-1 ml-2 mr-2 flex justify-between items-center text-xs text-gray-400">
                          {/* Horodatage à gauche */}
                  ...
                  <div className="flex flex-wrap gap-3">
                    {quickPrompts.map((prompt, index) => (
                      <button
                        ...
                        className="px-4 py-2 bg-white hover:bg-blue-50 text-gray-700 hover:text-blue-700 text-sm rounded-full border border-gray-200 hover:border-blue-200 transition-all duration-200 shadow-sm hover:shadow-md transform hover:scale-105"
                      >
                        {prompt}
                      </button>
                    ))}
              ...
              <div className="bg-white border-t border-gray-200 px-6 py-4">
                <div className="flex items-center justify-start gap-2">
                  ...
                  <div className="flex gap-2">
                    <button className="px-4 py-2 bg-blue-100 text-blue-700 rounded-lg text-sm font-medium border border-blue-200 hover:bg-blue-200 transition-colors">
                      💬 Conversation
                    </button>
                ...
                <form onSubmit={handleSubmit} className="flex gap-4">
                  <div className="flex-1 relative">
                    <input
                      ...
                      className="w-full bg-gray-50 text-gray-900 rounded-2xl px-6 py-4 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all border border-gray-200"
                      disabled={isLoading}
                    />
                  </div>
                  <button
